...
//   torch::Tensor max_contribute = torch::full({H, W}, 0.0, means3D.options().dtype(torch::kFloat32));
  ...
  torch::Tensor weightBuffer = torch::empty({0}, options.device(device));
  std::function<char*(size_t)> geomFunc = resizeFunctional(geomBuffer);
  std::function<char*(size_t)> binningFunc = resizeFunctional(binningBuffer);
  std::function<char*(size_t)> imgFunc = resizeFunctional(imgBuffer);
  std::function<char*(size_t)> weightFunc = resizeFunctional(weightBuffer);
	...
	const int mode) 
{
  ...
  if (mode==M_OURS) {
	...
	dL_dlanguage_feature_3d = torch::zeros({1}, means3D.options());
  }
